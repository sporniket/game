...
import com.sporniket.libre.game.papi.resource.ResourceNameProviderImage;

/**
 * Test suit for {@link ResourceNameProviderImage}.
 * 
 * <p>
 ...
 * @since 0-SNAPSHOT
 */
public class TestResourceNameProviderImage extends TestCase
{
	...
	public final void testBasicResource()
	{
		...
		ResourceNameProviderImage _provider = new ResourceNameProviderImage() ;
		...
		String _expected = "FHD"+File.separator+"test.png";
	...
	{
		ResourceLocalizable _testResource = new ResourceLocalizable("test", "png", new String[]{Locale.CANADA_FRENCH.toString(), Locale.FRANCE.toString()});
		ResourceNameProviderImage _provider = new ResourceNameProviderImage() ;
		ResourceNameContext _context = new ResourceNameContext(ScreenFeatureSet.Native.Landscape.FHD, Locale.FRANCE);
		String _result = _provider.getName(_testResource, _context);
		String _expected = "FHD"+File.separator+"test-fr_FR.png";
		if (!_result.equals(_expected))
		{
			fail("Expected : ["+_expected+"] ; result : ["+_result+"]");
		}
	...
	{
		ResourceLocalizable _testResource = new ResourceLocalizable("test", "png", new String[]{Locale.FRENCH.toString(), Locale.ENGLISH.toString()});
		ResourceNameProviderImage _provider = new ResourceNameProviderImage() ;
		ResourceNameContext _context = new ResourceNameContext(ScreenFeatureSet.Native.Landscape.FHD, Locale.GERMANY);
		String _result = _provider.getName(_testResource, _context);
		String _expected = "FHD"+File.separator+"test-en.png";
		if (!_result.equals(_expected))
		{
			fail("Expected : ["+_expected+"] ; result : ["+_result+"]");
		}
	...
	public final void testLocalizedResource__partiallyCompatible()
	{
		...
		ResourceNameContext _context = new ResourceNameContext(ScreenFeatureSet.Native.Landscape.FHD, Locale.FRANCE);
		String _result = _provider.getName(_testResource, _context);
		String _expected = "FHD"+File.separator+"test-fr.png";
		if (!_result.equals(_expected))
		{
			fail("Expected : ["+_expected+"] ; result : ["+_result+"]");
		}
